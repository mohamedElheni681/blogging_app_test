...
<div class="w-4/5 m-auto pt-3">
    <div class="card">
        <div class="card-body">
            <h3 class="card-title w-100"> {{__('post.create_post')}}</h3>
            <div class="card-text p-3">
                <form action="/blog" method="POST" class="form" id="post_form">
                    @csrf

                    <div class="mb-3">
                        <label class="required mb-2">{{__('post.title')}}</label>
                        <input type="text" name="title" id="title" value="{{ old('title', '') }}" placeholder="{{__('post.add_post_title')}}" class="form-control">
...
@section('js')
    <script type="">
        function setEditorOldDescription(quill) {
            value ="{{old('description') ? old('description') : ''}}"
            if (value != "") {
                quill.setHTML(value);
                quill.setHTML($('.ql-editor').text());
            }
        }
